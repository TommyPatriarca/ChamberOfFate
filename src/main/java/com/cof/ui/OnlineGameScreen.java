...
public class OnlineGameScreen {
    ...
    public void show(Stage primaryStage) {
        ...
        new Thread(() -> {
            while (true) {
                try {
                    // Ottieni lo stato attuale del turno dal server
                    boolean isMyTurn = controller.isMyTurn();

                    // Aggiorna l'interfaccia utente sul thread principale
                    Platform.runLater(() -> {
                        setPlayerControlsEnabled(isMyTurn); // Abilita i controlli se è il tuo turno
                        currentPlayerLabel.setText(isMyTurn ? "Your Turn" : "Opponent's Turn");
                    });

                    // Tempo di attesa prima di controllare nuovamente
                    Thread.sleep(1000);
                } catch (Exception e) {
                    e.printStackTrace();
                }
